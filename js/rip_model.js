...
function onButton ()
{
	...
	if(button.addEventListener){
         button.addEventListener("click", function() { getUIdata();});
    } else {
         button.attachEvent("click", function() { getUIdata();});
    };
}

function initUI()
{	
	// Get UI objects from HTML file
	K_textfields[0] = document.getElementById("k1");
	K_textfields[1] = document.getElementById("k2");
	K_textfields[2] = document.getElementById("k3");
	K_textfields[3] = document.getElementById("k4");
	X_textfield = document.getElementById("gain_X");
	backlashCheckbox = document.getElementById("backlash_checkbox");
}
...
function getUIdata ()
{
	...
	ripCanvas.width = (document.getElementById('MainPageArea').clientWidth-20);
	
	// Parse UI inputs
	for (n=0; n<4; n++)
	{
		K[n] = K_textfields[n].value;
	}
	gainX = X_textfield.value;
	backlashFlag = backlashCheckbox.checked;
	
	// Reinitialise outputs, too
...
function animate(canvas, context)
{
	...
	for (n=0; n < SAMPLES_PER_FRAME; n++)
	{
		...
		outputs = ripTransition(Vm, outputs, Ts, backlash);
